...
def setupUMLEstGuessRate(model="Logistic", swptRule="Up-Down", nDown=2, centTend="Mean", stimScale="Linear", x0=None, xLim=(-10, 10), 
                         ...
                         gammaLim=(0,0.2), gammaStep=0.01, gammaSpacing="Linear", gammaDist="Uniform", gammaMu=0, gammaSTD=0.1,
    ...
    UML["par"]["gamma"] = {}
    UML["par"]["gamma"]["limits"] = gammaLim
    UML["par"]["gamma"]["step"] = gammaStep
    #UML["par"]["gamma"]["scale"] = gammaScale
    UML["par"]["gamma"]["spacing"] = gammaSpacing
    UML["par"]["gamma"]["dist"] = gammaDist
    UML["par"]["gamma"]["mu"] = gammaMu
    UML["par"]["gamma"]["std"] = gammaSTD
    ...
    if len(UML["alpha"]) > 1: #alpha is not fixed, estimate it
        UML["swpts_idx"] = np.append(UML["swpts_idx"], 2)
    if len(UML["beta"]) > 1: #beta is not fixed, estimate it
        UML["swpts_idx"] = np.append(UML["swpts_idx"], [1,3])
    if len(UML["gamma"]) > 1: #gamma is not fixed, estimate it
        UML["swpts_idx"] = np.append(UML["swpts_idx"], 0)
    if len(UML["lambda"]) > 1: #lambda is not fixed, estimate it
        UML["swpts_idx"] = np.append(UML["swpts_idx"], 4)
...
def setP0(UML):
    ...
    (UML["a"], UML["b"], UML["g"], UML["l"]) = meshgrid(UML["alpha"], UML["beta"], UML["gamma"], UML["lambda"], indexing='ij')
...
def UMLEstGuessRate_update(UML, r):
    UML["n"] = UML["n"] +1
    ...
    if UML["par"]["model"] == "Logistic":
        UML["p"] = UML["p"] + \
                   log(prepare_prob(logisticPsy(UML["xnext"], UML["a"], UML["b"], UML["g"], UML["l"]))**r) + \
                   log(prepare_prob(1-logisticPsy(UML["xnext"], UML["a"], UML["b"], UML["g"], UML["l"]))**(1-r))
    elif UML["par"]["model"] == "Gaussian":
        UML["p"] = UML["p"] + \
                   log(prepare_prob(gaussianPsy(UML["xnext"], UML["a"], UML["b"], UML["g"], UML["l"]))**r) + \
                   log(prepare_prob(1-gaussianPsy(UML["xnext"], UML["a"], UML["b"], UML["g"], UML["l"]))**(1-r))
    elif UML["par"]["model"] == "Weibull":
        UML["p"] = UML["p"] + \
                   log(prepare_prob(weibullPsy(UML["xnext"], UML["a"], UML["b"], UML["g"], UML["l"]))**r) + \
                   log(prepare_prob(1-weibullPsy(UML["xnext"], UML["a"], UML["b"], UML["g"], UML["l"]))**(1-r))
    ...
    if UML["par"]["method"] == "Mode":
        ...
        idx = np.argmax(UML["p"])
        if UML["n"] < 2:
            UML["phi"] = np.array([ravel(UML["a"])[idx], ravel(UML["b"])[idx], ravel(UML["g"])[idx], ravel(UML["l"])[idx]], ndmin=2)
        else:
            UML["phi"] = np.concatenate((UML["phi"], np.array([ravel(UML["a"])[idx], ravel(UML["b"])[idx], ravel(UML["g"])[idx], ravel(UML["l"])[idx]], ndmin=2)), axis=0)
        ...
        gamma_est_tmp = np.sum(pdf_tmp*UML["g"])
        lambda_est_tmp = np.sum(pdf_tmp*UML["l"])

        if UML["n"] < 2:
            UML["phi"] = np.array([alpha_est_tmp, beta_est_tmp, gamma_est_tmp, lambda_est_tmp], ndmin=2)
        else:
            UML["phi"] = np.concatenate((UML["phi"], np.array([alpha_est_tmp, beta_est_tmp, gamma_est_tmp, lambda_est_tmp], ndmin=2)), axis=0)
    ...
    UML["est_guess"] = UML["phi"][UML["phi"].shape[0]-1, 2]
    ...
    if UML["par"]["model"] == "Logistic":
        xCeiling = invLogisticPsy(UML["par"]["lambdaSwptPC"]-UML["est_lapse"], est_alpha, UML["est_slope"], UML["est_guess"], UML["est_lapse"])
    elif UML["par"]["model"] == "Gaussian":
        xCeiling = invGaussianPsy(UML["par"]["lambdaSwptPC"]-UML["est_lapse"], est_alpha, UML["est_slope"], UML["est_guess"], UML["est_lapse"])
    elif UML["par"]["model"] == "Weibull":
        xCeiling = invWeibullPsy(UML["par"]["lambdaSwptPC"]-UML["est_lapse"], est_alpha, UML["est_slope"], UML["est_guess"], UML["est_lapse"])
   
    lmbdSwpt = min(max(UML["par"]["suggestedLambdaSwpt"], xCeiling), UML["par"]["x"]["limits"][1])
    
    #swpt = np.append(swpt, UML["par"]["x"]["limits"][1])
    swpt = np.insert(arr=swpt, obj=0, values=UML["par"]["x"]["limits"][0]) #gamma sweetpoint
    ...
    if UML["par"]["swptRule"] == "Up-Down":
        ...
        newx = swpt[random.choice([0,1,2,3,4])]
