...
def PSI_select_next_stim(PSI):
    ...
    pCorrNextNorm = np.zeros((len(PSI["stims"]), PSI["p"].shape[0], PSI["p"].shape[1], PSI["p"].shape[2]))
    pIncorrNextNorm = np.zeros((len(PSI["stims"]), PSI["p"].shape[0], PSI["p"].shape[1], PSI["p"].shape[2]))
    ...
    pCorrNextScaler = np.zeros(len(PSI["stims"]))
    pIncorrNextScaler = np.zeros(len(PSI["stims"]))
    #entropy for each possible stimulus level in case of a correct and an incorrect response
    entrCorr = np.zeros(len(PSI["stims"]))
    entrIncorr = np.zeros(len(PSI["stims"]))
    for i in range(len(PSI["stims"])):
