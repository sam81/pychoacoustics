...
from pychoacoustics.PSI_method_est_guess import*
import time
t1 = time.time()
...
marginalize_opts = [None , tuple(np.array([0])), tuple(np.array([1])), tuple(np.array([2])),
                    tuple(np.array([3])), tuple(np.array([1,2])), tuple(np.array([1,3])),
                    tuple(np.array([2,3])), tuple(np.array([1,2,3]))]
...
for margOpt in range(len(marginalize_opts)):
     ...
     PSI = setupPSI(model="Logistic", stimScale="Linear", x0=None, xLim=(-20, 20), xStep=1, 
                    ...
                    gammaLim=(0,0.3), gammaStep=0.01, gammaSpacing="Linear", gammaDist="Uniform", gammaMu=0.1, gammaSTD=0.1,
     ...
     for i in range(nTrials):
          resp = virtualObserver(PSI["xnext"], midpoint=0, slope=2, guess=0.15, lapse=0, funType="Logistic", funFit="Linear")
          resps[i] = resp
          PSI = PSI_update(PSI, resp)


     np.savetxt(rootPath+'res_logistic_test_gamma'+str(testN)+'.txt', PSI["phi"])
     np.savetxt(rootPath+'resp_logistic_test_gamma'+str(testN)+'.txt', resps)
...
for margOpt in range(len(marginalize_opts)):
     ...
     resps = np.zeros((nTrials))
     for i in range(nTrials):
          resp = virtualObserver(exp(PSI["xnext"]), midpoint=log(5), slope=2, guess=0.15, lapse=0, funType="Logistic", funFit="Logarithmic")
          resps[i] = resp
          PSI = PSI_update(PSI, resp)
...
if runChecks == True:
     for i in range(testN):
          res_pal = np.loadtxt(rootPath+'res_pal_logistic_test_gamma'+str(i+1)+'.txt', delimiter=',')
          res_pyth = np.loadtxt(rootPath+'res_logistic_test_gamma'+str(i+1)+'.txt')
